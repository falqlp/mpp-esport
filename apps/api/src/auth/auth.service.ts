...
export class AuthService {
  ...
  async register(input: { email?: string; displayName?: string; password?: string }) {
    ...
    if (displayName.length < 2 || password.length < 8) throw new BadRequestException('Nom trop court ou mot de passe de moins de 8 caractères');
    if (await this.prisma.user.findUnique({ where: { email } })) throw new BadRequestException('Cette adresse email est déjà utilisée');
    const user = await this.prisma.user.create({ data: { email, displayName, passwordHash: await this.hashPassword(password) } });
  ...
  async login(input: { email?: string; password?: string }) {
    ...
    if (!user || !await this.verifyPassword(input.password ?? '', user.passwordHash)) throw new UnauthorizedException('Email ou mot de passe incorrect');
    return this.session(user);
  }
  ...
  async authenticate(authorization?: string): Promise<AuthUser | undefined> {
    ...
    if (!payload || !signature || signature.length !== expectedSignature.length ||
      !timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))) return undefined;
    try {
